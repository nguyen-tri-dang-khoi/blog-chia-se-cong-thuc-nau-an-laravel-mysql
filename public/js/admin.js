$( function() {
    ...
    $(document).on('click','#upload_image',function(e){
      ...
      let url = window.location.protocol + "//" +window.location.hostname + ":" + window.location.port+'/admin/upload_image';
      ...
      $.ajax({
        ...
        success:function(data){
          ...
          let image = '<img src="' + url_image + data.photo+ '"' + " class='tm-product-img-dummy' id='display-image'/>";
          if($('#where_replace2').length > 0){
            $('#where_replace2').replaceWith(image);
          } else {
            $('#where_replace').replaceWith(image);
          }
          alert('Ngài đã tải ảnh lên thành công');
        },
        error:function(data){
    ...
    $(document).on('click','#upload_profile',function(e){
      ...
      let url = window.location.protocol + "//" +window.location.hostname + ":" + window.location.port+'/admin/upload_profile';
     ...
     if(password === confirm_password){
      ...
      $.ajax({
        ...
        processData: false,
        success:function(data){
          let url_image = window.location.protocol + "//" +window.location.hostname + ":" + window.location.port + "/admin";
        },
        error:function(data){
          console.log("Error: " , data);
        }
      ...
      if(confirm("Bạn có chắc chắn muốn xoá tài khoản ?"))
      {
        $.ajax({
          ...
          success:function(data){
            ...
            location.replace(window.location.protocol + "//" +window.location.hostname + ":" + window.location.port+"/login");
          },
          error:function(data){
            console.log("Error: " , data);
          }
